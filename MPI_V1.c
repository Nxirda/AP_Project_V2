...
void handler(const char* filename, void(*hash_function)(const u8 *, const u64, u8 *), u64 nt, u64 hash_len, ascii *password, u8 *target_hash, i32 rank, i32 size)
{
    ...
    off_t chunk_size = file_size / size;
    off_t offset = rank * chunk_size;

    //Align offset and chunk to page size for mmap
    off_t page_size = sysconf(_SC_PAGE_SIZE);    
    chunk_size = (chunk_size / page_size) * page_size;
    offset = (offset /page_size)* page_size;

    //Mapping chunk of the file to page
    ascii *file_data = mmap(NULL, chunk_size, PROT_READ, MAP_PRIVATE, fd, offset);
    ...
    //Sync all the process before the lookup
...
int main(int argc, char **argv)
{
    ...
    u8 any_process_found = 0;
    MPI_Allreduce(MPI_IN_PLACE, &any_process_found, 1, MPI_UNSIGNED_CHAR, MPI_MAX, MPI_COMM_WORLD);
